...
//	goto sigue;
	...
	if (register_chrdev_region(ncdd_dev_t, 26, "ncdd")) {
//	if (register_chrdev_region(ncdd_dev_t, 256, DEVICE_NAME)) {
...
//ncdd_cdev.ops = &Fops;
err = cdev_add (&ncdd_cdev, ncdd_dev_t, 26);
/* Fail gracefully if need be */
if (err) {
	printk(KERN_NOTICE "Error %d adding ncdd", err);

	unregister_chrdev_region(ncdd_dev_t, 26);
	return err;
}
#ifdef DEBUG
	printk ("NCDD: added\n");
