...
struct task_struct *proces_client; //proces client pare
...
(void)
{
...
#ifdef DEBUG
			printk ("NCDD: Afegida peticio (lliure) direccio: %p\n",p);
#endif
			return p;
    	}
		anterior=p;
	...
	anterior->seguent=p;
#ifdef DEBUG
	printk ("NCDD: Afegida peticio (nova) direccio: %p\n",p);
#endif
...
(pid_t pid,struct file *file)
{
	...
	for (i=0;i<NCDD_MAX_CLIENTS_FILL;i++) {
		if (fills_client[i].pid==0) { //Afegir-ho aqui
			...
			if (buffer_intercanvi==NULL) {
				printk ("NCDD: Error al assignar buffer d'intercanvi pel client\n");
				return -1;
			}
...
#ifdef DEBUG
			printk ("NCDD: Afegit client fill pid %d a la posicio %d , buffer_intercanvi: %p\n"
	...
	printk ("NCDD: S'ha arribat al maxim de fills del client\n");
	return -1;
}
...
#ifdef DEBUG
				printk ("NCDD: Client fill trobat file:%p pid:%d\n",
...
#ifdef DEBUG
	printk ("NCDD: No s'ha trobat el client fill de file:%p\n",file);
#endif
	return NULL;
...
(int pid)
{
	...
	//Error, fill no trobat
#ifdef DEBUG
	printk ("NCDD: No s'ha trobat el client fill de pid:%u\n",pid);
#endif
	return NULL;
...
(int i)
{
	...
	if (fills_client[i].pid!=0) {
#ifdef DEBUG
		printk ("NCDD: Client fill trobat file:%p pid:%d buffer_intercanvi:%p->Eliminat\n",
...
(struct file *file)
{
	...
	//Error, fill no trobat
#ifdef DEBUG
	printk ("NCDD: No s'ha trobat el client fill de file:%p per eliminarlo\n",file);
#endif
	return -1;
}
...
#ifdef DEBUG
	printk ("NCDD: Dispositiu obert (inode:%p,file:%p)\n", inode, file);
	printk("NCDD: Dispositiu %d.%d\n",
...
*/
...
#ifdef DEBUG
		printk ("NCDD: Peticio open minor: %d, file: %p\n",minor,file);
		printk ("NCDD: f_flags:%u f_mode:%o\n",file->f_flags,file->f_mode);
#endif

		//Despertem proces pare de la senyal ioctl
#ifdef DEBUG
		printk ("NCDD: Despertem client pare per atendre una ordre open\n");
#endif
		...
		if (kill_proc(proces_client->pid,NCDD_SENYAL_PETICIO_ALTRES,1)<0) {
			...
			printk ("NCDD: Error, no s'ha pogut avisar al client\n");
			return -EINVAL;
		}

		//Se despierta el padre con señales, para poder dormir despues
...
//		wake_up_process(proces_client);
	//	schedule();
...
#ifdef DEBUG
		printk ("NCDD: Dormir proces que espera un retorn d'ordre OPEN\n");
#endif
		...
		if (peticio->estat==NCDD_PETICIO_RETORNADA) {
			...
			valor_retorn=-EINTR;
#ifdef DEBUG
			printk ("NCDD: Ordre open interrompuda\n");
#endif
			...
			if (pid<0) {
				printk ("NCDD: No s'ha trobat el client fill de file:%p al fer close\n",file);
				return -EIO;
			}
...
#ifdef DEBUG
			printk ("NCDD: Eliminar client fill ja que open ha retornat error\n");
#endif
			if (eliminar_fill(file)<0) {
#ifdef DEBUG
				printk ("NCDD: Error al intentar eliminar fill amb file %p\n",file);
#endif
			}
			return valor_retorn;
		}
...
#ifdef DEBUG
  printk ("NCDD: Dispositiu liberat (inode:%p,file:%p)\n", inode, file);
#endif


	if (current->pid==proces_client->pid) {
		printk ("NCDD: Client terminat.\n");
...
*/
...
#ifdef DEBUG
		printk ("NCDD: Peticio close minor: %d, file: %p, pid: %d\n",minor,file,current->pid);
#endif
		...
		if (peticio->estat==NCDD_PETICIO_RETORNADA) {
			...
			valor_retorn=-EINTR;
#ifdef DEBUG
			printk ("NCDD: Ordre close interrompuda\n");
#endif
...
*/
		...
		*p=NCDD_ORDRE_READ;
		...
		if (client==NULL) {
#ifdef DEBUG
			printk ("NCDD: No s'ha trobat el client fill de file:%p al fer read\n",file);
#endif
			return -EIO;
		}
		pid=client->pid;

		//Bucle que descomposa les dades en troços iguals a NCDD_TAMANY_BUFFER_INTERCANVI
		do {
...
#ifdef DEBUG
			printk ("NCDD: Peticio read minor: %d, file: %p, pid: %d\n",minor,file,current->pid);
			printk ("NCDD: longitut:%d\n",longitut_fragment);
#endif
			...
			if (peticio->estat==NCDD_PETICIO_RETORNADA) {
...
#ifdef DEBUG
					printk ("NCDD: Movent %d dades de %p a %p de resultat de read\n",
				...
				valor_retorn=-EINTR;
#ifdef DEBUG
				printk ("NCDD: Ordre read interrompuda\n");
#endif
				peticio->estat=NCDD_PETICIO_LLIURE;
...
*/
		...
		if (client==NULL) {
#ifdef DEBUG
			printk ("NCDD: No s'ha trobat el client fill de file:%p al fer write\n",file);
#endif
			return -EIO;
		}
		pid=client->pid;

		do { //Bucle que descomposa les dades en troços iguals a NCDD_TAMANY_BUFFER_INTERCANVI
			if (length-longitut_total>NCDD_TAMANY_BUFFER_INTERCANVI)
				longitut_fragment=NCDD_TAMANY_BUFFER_INTERCANVI;
			else longitut_fragment=length-longitut_total;
			...
			*p=NCDD_ORDRE_WRITE;
...
#ifdef DEBUG
			printk ("NCDD: Peticio write minor: %d, file: %p, pid: %d\n",minor,file,current->pid);
			printk ("NCDD: longitut:%d\n",longitut_fragment);
#endif
			...
			if (longitut_fragment) {
				//Traspassar dades del client_fill cap al proces que ha fet la peticio
#ifdef DEBUG
				printk ("NCDD: Movent %u dades de %p a %p de write\n",
			...
			if (peticio->estat==NCDD_PETICIO_RETORNADA) {
				...
				valor_retorn=-EINTR;
#ifdef DEBUG
				printk ("NCDD: Ordre write interrompuda\n");
#endif
				peticio->estat=NCDD_PETICIO_LLIURE;
...
*/
			...
			if (ncdd_estat_control==NCDD_ESTAT_CONTROL_INICI) {
				...
				for (;index_identificacio<(sizeof(NCDD_IDENTIFICACIO))+1;
					...
					if (c!=identificacio[index_identificacio]) {
						printk ("NCDD: Identificacio no valida\n");
						index_identificacio=0;
				...
				printk ("NCDD: Client registrat amb pid: %d\n",proces_client->pid);
				...
				printk ("NCDD: Client ja registrat!\n");
				return -EINVAL;
			}
		}
		...
		if (cmd==NCDD_IOCTL_LLEGIR_OPEN) {
			if (current->pid!=proces_client->pid) {
				printk ("NCDD: Client no autoritzat\n");
				return -EINVAL;
			}
...
#ifdef DEBUG
				printk ("NCDD: Client pare dormint\n");
#endif
...
#ifdef DEBUG
			printk ("NCDD: Intentant Registrar fill PID:%d f:%p\n",pid,f);
#endif

			//Posar peticio open a estat pendent de retorn, que correspongui a f
#ifdef DEBUG
			printk ("NCDD: Buscant peticio pendent file:%p\n",f);
#endif
...
int init_module()
{
	...
	if (register_chrdev(Major,DEVICE_NAME,&Fops)<0) {
		printk ("NCDD: Error registrant dispositiu NCDD amb numero major %d\n",Major);
		return Major;
	}
